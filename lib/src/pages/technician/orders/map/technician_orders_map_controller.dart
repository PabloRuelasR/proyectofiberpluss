...
import 'package:proyectofiberpluss/src/environment/environment.dart';
...
class TechnicianOrdersMapController extends GetxController {

  Socket socket = io('${Environment.API_URL}orders/technician', <String, dynamic> {
  ...
  CameraPosition initialPosition = CameraPosition(
      target: LatLng(-12.0949479, -76.7815755),
      zoom: 14
  );
  ...
  BitmapDescriptor? technicianMarker;
  ...
  void isCloseToTechnicianPosition() {
  ...
  void emitToTechnician() {
    socket.emit('technician', {
      'id_order': order.id,
    });
  }
  ...
  void checkGPS() async {
    technicianMarker = await createMarkerFromAssets('assets/img/delivery_little.png');
  ...
  void updateToTechnician() async {
    if (distanceBetween <= 200) {
      ResponseApi responseApi = await ordersProvider.updateToTechnician(order);
      Fluttertoast.showToast(msg: responseApi.message ?? '', toastLength: Toast.LENGTH_LONG);
      if (responseApi.success == true) {
        emitToTechnician();
        Get.offNamedUntil('/technician/home', (route) => false);
      }
    }
    else {
  ...
  void updateLocation() async {
    try{
      ...
      saveLocation();
      animateCameraPosition(position?.latitude ?? -12.0949479, position?.longitude ?? -76.7815755);

      addMarker(
          'technician',
          position?.latitude ?? -12.0949479,
          position?.longitude ?? -76.7815755,
          'Tu posicion',
          '',
          technicianMarker!
      );
      ...
      addMarker(
          'home',
          order.address?.lat ?? -12.0949479,
          order.address?.lng ?? -76.7815755,
          'Lugar de entrega',
          '',
          homeMarker!
      );
      ...
      LatLng to = LatLng(order.address?.lat ?? -12.0949479, order.address?.lng ?? -76.7815755);
